...
- Configurable cross-role pod anti-affinity via `spec.affinity` (opt-in; unchanged when unset): `mode: segment-mirror` keeps each segment primary off its mirror's node (best-effort/preferred), `mode: full` also spreads coordinator, standby, segment primaries/mirrors, and the backup Job onto distinct nodes. Per-field `*bool` toggles override the preset; `type: required` is honored for the coordinator↔standby and coordinator↔backup terms but downgraded to preferred (with an admission Warning) for the segment primary↔mirror term. `topologyKey` defaults to `kubernetes.io/hostname`. Operator-generated terms are additive — user-supplied affinity is preserved.
- `spec.standby.tolerations` allow scheduling the standby coordinator onto tainted nodes (parity with coordinator, segments, and backup)
