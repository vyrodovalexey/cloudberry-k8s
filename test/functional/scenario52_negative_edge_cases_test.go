...
func (s *Scenario52NegativeEdgeCaseSuite) TestFunctional_Scenario52g_BasicAuthFallback() {
	...
	t.Run("bearer_token_rejected_without_oidc", func(t *testing.T) {
		...
		// C3 hardening: the 401 body is GENERIC — it must not disclose
		// which providers are (un)configured. The concrete reason lives in
		// logs/spans only; the client gets the RFC 7235 challenge advertising
		// only the configured scheme (Basic here, since OIDC is nil).
		body, err := io.ReadAll(resp.Body)
		require.NoError(t, err)
