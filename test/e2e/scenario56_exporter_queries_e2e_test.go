...
// TestE2E_Scenario56e_TableStatistics verifies that queries.yaml contains the
// CATALOG-ONLY cloudberry_table_stats query (cycle-2 fix): labels schemaname,
// relname and the estimate metrics n_live_tup + table_size_bytes sourced from
// pg_class/pg_namespace. On Cloudberry pg_stat_user_tables is a DISTRIBUTED
// view (every scrape dispatches to all segments over the Motion/Interconnect
// layer), so the per-table DML/scan counters (seq_scan, n_tup_ins, ...) were
// deliberately dropped — a sidecar exporter must never trigger cluster-wide
// dispatch per scrape. Also verifies LIMIT 100 in query.
