...
// ============================================================================
// Scenario 57: Query Exporter Metrics (E2E)
// ============================================================================
//
...
// Scenario57QueryExporterMetricsE2ESuite tests the cloudberry-query-exporter
// sidecar container configuration and collectors.go metric definitions.
type Scenario57QueryExporterMetricsE2ESuite struct {
	suite.Suite
}

func TestE2E_Scenario57(t *testing.T) {
	t.Parallel()
	suite.Run(t, new(Scenario57QueryExporterMetricsE2ESuite))
}

// scenario57E2ECluster returns a cluster with query monitoring and cloudberry-query-exporter
// enabled for testing sidecar container configuration.
func scenario57E2ECluster() *cbv1alpha1.CloudberryCluster {
...
// TestE2E_Scenario57a_QueryActivityMetrics verifies that the
...
func (s *Scenario57QueryExporterMetricsE2ESuite) TestE2E_Scenario57a_QueryActivityMetrics() {
	cluster := scenario57E2ECluster()
...
// TestE2E_Scenario57b_ResourceGroupCollector verifies that collectors.go
...
func (s *Scenario57QueryExporterMetricsE2ESuite) TestE2E_Scenario57b_ResourceGroupCollector() {
...
// TestE2E_Scenario57c_ResourceGroupIOCollector verifies that collectors.go
...
func (s *Scenario57QueryExporterMetricsE2ESuite) TestE2E_Scenario57c_ResourceGroupIOCollector() {
	content, err := os.ReadFile("../../cmd/cloudberry-query-exporter/collectors.go")
	require.NoError(s.T(), err)
	src := string(content)
...
// TestE2E_Scenario57d_SpillFileCollector verifies that collectors.go
...
func (s *Scenario57QueryExporterMetricsE2ESuite) TestE2E_Scenario57d_SpillFileCollector() {
	content, err := os.ReadFile("../../cmd/cloudberry-query-exporter/collectors.go")
	require.NoError(s.T(), err)
	src := string(content)
...
// TestE2E_Scenario57e_SegmentHealthCollector verifies that collectors.go
...
func (s *Scenario57QueryExporterMetricsE2ESuite) TestE2E_Scenario57e_SegmentHealthCollector() {
	content, err := os.ReadFile("../../cmd/cloudberry-query-exporter/collectors.go")
	require.NoError(s.T(), err)
	src := string(content)
...
// TestE2E_Scenario57f_DistributedTransactionCollector verifies that collectors.go
...
func (s *Scenario57QueryExporterMetricsE2ESuite) TestE2E_Scenario57f_DistributedTransactionCollector() {
	content, err := os.ReadFile("../../cmd/cloudberry-query-exporter/collectors.go")
	require.NoError(s.T(), err)
	src := string(content)
...
// TestE2E_Scenario57g_SkewCollector verifies that collectors.go
// contains data distribution skew metric definitions: table_skew_coefficient,
// and the gp_toolkit.gp_skew_coefficients query.
func (s *Scenario57QueryExporterMetricsE2ESuite) TestE2E_Scenario57g_SkewCollector() {
	content, err := os.ReadFile("../../cmd/cloudberry-query-exporter/collectors.go")
	require.NoError(s.T(), err)
	src := string(content)
