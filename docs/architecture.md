...
| NetworkPolicy | `{cluster}-pxf` | Created only when the PXF sidecar is enabled (SE.5): seals cross-pod PXF `:5888` while allowlisting the service/exporter ports and — from same-cluster pods only — the MPP interconnect ephemeral ranges (TCP split around 5888, full UDP) plus TCP 22 for coordinator→segment SSH |
...
- **Deletion is handled first** — `Reconcile` checks `DeletionTimestamp` before the action-annotation gate, the lifecycle-phase short-circuit, and the generation skip, so a Terminating cluster reaches `handleDeletion` (and gets its finalizer removed) from **any** phase. Previously a cluster deleted while `Stopped` returned before deletion handling and stayed Terminating forever, and `Restricted`/`Maintenance` clusters requeued endlessly
...
- **Steady-state readiness refresh** — even when the generation is unchanged, the reconciler re-reads coordinator/standby/segment readiness from the live StatefulSets and persists `coordinatorReady`/`standbyReady`/`segmentsReady` when they drift (explicit MergePatch so `omitempty` zero values are kept), so a mid-steady-state pod restart can't freeze a stale count in status
...
- **Reconciliation metrics**: `cloudberry_reconcile_total`, `cloudberry_reconcile_errors_total`, `cloudberry_reconcile_duration_seconds` — emitted by **all four controllers** (cluster, admin, HA, auth) via a shared `recordReconcileOutcome` helper, not just the cluster controller — plus `cloudberry_steady_state_refresh_errors_total` (labels `cluster`, `namespace`, `component` ∈ {`backup`, `dataloading`, `storage`}): swallowed (non-fatal) errors on the admin controller's steady-state refresh paths, which intentionally don't change reconcile results but are now alertable instead of WARN-log-only
...
- **Query-exporter self-observability** (`cmd/cloudberry-query-exporter`, exposed on the coordinator sidecar's own `/metrics`, separate from the operator endpoint): `cbexporter_collector_errors_total` (label `collector`) and `cbexporter_collector_duration_seconds` (label `collector`) — a per-collector scrape error counter + duration, with `collector` ∈ {`query_activity`, `resgroup_status`, `resgroup_iostats`, `spill_files`, `segment_health`, `dist_txns`, `table_skew`} — plus `cloudberry_query_exporter_history_errors_total` (label `stage` ∈ {`snapshot`, `insert`, `explain`}) for the query-history pipeline: the history INSERT (5 s) and hourly retention DELETE (60 s) are deadline-bounded because they target the distributed `cloudberry_query_history` table, so a degraded interconnect fails fast onto this counter instead of wedging the collect loop. The unbounded `usename` label was **removed** from `cbexporter_queries_total` / `cbexporter_queries_slow_total` to bound metric cardinality
