...
- **What's verified** (the `pxf` container injected when `dataLoading.enabled && pxf.enabled && pxf.image != ""`):
  ...
  - **Liveness probe**: `HTTPGet /actuator/health` on `5888`, `initialDelaySeconds: 60`, `periodSeconds: 20`, with a more tolerant `timeoutSeconds`. **Readiness probe**: `HTTPGet /actuator/health` on `5888`, `initialDelaySeconds: 30`, `periodSeconds: 10`. **Startup probe**: `HTTPGet /actuator/health` on `5888`, `periodSeconds: 5`, `failureThreshold: 24` (a ~120 s startup budget), so the slow ~50 s Spring Boot cold start completes before liveness is evaluated and never trips the sidecar into `CrashLoopBackOff`.
...
- **The 5 checks (HC.1-HC.5), gated:**
  - **HC.1** PXF readiness — pxf jobs when `pxf.enabled`: a `psql` **DB-PROXY** probe (`SELECT 1` → `SELECT 1 FROM pg_extension WHERE extname='pxf'` → `SELECT 1 FROM pg_proc WHERE proname = 'pxf_read'`). PXF 2.1 ships **no** `pxf_version()` SQL function, so the readiness step checks the catalog for a real PXF function (`pxf_read`) to prove PXF is actually usable. **HC.1 DB-proxy honesty:** the load pod **CANNOT** reach a segment's localhost-only PXF sidecar (segment↔sidecar is `localhost`-only), so HC.1 is a DB proxy, **NOT** a direct curl of the sidecar `/actuator/health`. The segment-pod sidecar **liveness probe** uses `/actuator/health`; `/pxf/v15/Status` is the legacy path that **404s** and must NOT be used. The LIVE proof of HC.1 is "stop PXF on a segment → the job fails".
  - **HC.2** target table exists — ALL jobs: `psql … to_regclass('<targetTable>')`. The DETERMINISTIC headline (drop the table → fail).
  - **HC.3** source connectivity — pxf **object-store** jobs: captures the HTTP status code from `${AWS_S3_ENDPOINT}/` (`curl -sS -o /dev/null -w '%{http_code}'`, **without** `-f`) and accepts ANY HTTP response (1xx–5xx) as reachable. MinIO and many S3-compatible stores answer an unauthenticated HEAD/GET with **400/403**, so `curl -f` (which fails on 4xx) wrongly reported a reachable endpoint as "unreachable"; only a true connection failure/timeout now fails. **Skipped** for `jdbc`/`hive`/`hbase`/`hdfs`.
...
#### Scenario 113 — Storage Recommendations Validation Rules (Negative Tests) (W.1–W.4)

The systematic rejected-CR negative-test matrix for the **storage-recommendations** webhook rules **W.1–W.4** on `spec.storage.recommendationScan` (`RecommendationScanSpec`). These `W.*` IDs are **DISTINCT** from the data-loading `W.1`–`W.25` family ([Scenario 89](#scenario-89--pxf-data-loading-webhook-validation-all-rules) / [Scenario 110](#scenario-110--webhook-validation-all-rules-w1w15)); the two `W.*` namespaces do not overlap. The rules live in `internal/webhook/validating.go::validateStorageManagement`, are **gated on `recommendationScan.enabled: true`**, and are **webhook-authoritative** (no CRD `Minimum`/`Maximum` markers — static markers cannot honor the `enabled`-gate and would replace the descriptive, field-specific message with a generic apiserver range error).

- **W.1 — `bloatThreshold` must be 0–100** → `storage.recommendationScan.bloatThreshold must be between 0 and 100, got <n>` (113a: `150` and `-1` reject).
- **W.2 — `skewThreshold` must be 0–100** → `storage.recommendationScan.skewThreshold must be between 0 and 100, got <n>` (113b: `101` rejects).
- **W.3 — `indexBloatThreshold` must be 0–100** → `storage.recommendationScan.indexBloatThreshold must be between 0 and 100, got <n>` (113c: `200` rejects).
- **W.4 — `ageThreshold` must be non-negative** → `storage.recommendationScan.ageThreshold must be non-negative, got <n>` (113d: `-5` rejects).
- **Boundary ACCEPTS (CONTROL — no false-positive):** bloat/skew/indexBloat `= 0` and `= 100`; age `= 0` admit and persist.
- **Contract:** every rejection names the field path **and** the offending value; a rejected CR does **NOT** persist (a follow-up `GET` is `NotFound`); denials increment `cloudberry_webhook_admission_total{webhook="validating",result="denied"}` (no dedicated per-rule storage metric). **Verified live** against the deployed Vault-PKI validating webhook (all four rejects with the exact messages + `NotFound` on `GET`; the boundary CR created successfully).
- **Artifacts**: `internal/webhook/scenario113_validation_test.go` (unit), `test/cases/scenario113_storage_validation_cases.go` (catalog), `test/functional/scenario113_storage_validation_test.go`, `test/integration/scenario113_storage_validation_test.go`, `test/e2e/scenario113_storage_validation_e2e_test.go`.

See [spec 13 §Scenario 113](../specifications/13-storage-recommendations-spec.md#scenario-113--validation-rules-negative-tests).

#### Scenario 114 — Storage Recommendations Mutating Webhook Defaults (D.1–D.6)

The systematic default-injection matrix for the **storage-recommendations** mutating-webhook rules **D.1–D.6** on `spec.storage.recommendationScan` (`RecommendationScanSpec`). These `D.*` IDs are **DISTINCT** from the data-loading defaults `D.1`–`D.14` ([Scenario 90](#scenario-90--pxf-data-loading-webhook-defaults) / `setDataLoadingDefaults`); the two `D.*` namespaces do not overlap. The defaults live in `internal/webhook/mutating.go::setStorageManagementDefaults` (reached via the public `Default()` entrypoint), are **gated on `recommendationScan.enabled: true`**, and are **webhook-authoritative** (no CRD per-field `+kubebuilder:default` markers — static markers cannot honor the `enabled`-gate).

- **D.1 — `schedule`** → `0 3 * * 0` (weekly Sunday 3 AM; injected only when empty).
- **D.2 — `bloatThreshold`** → `20` (injected only when `0`).
- **D.3 — `skewThreshold`** → `50` (injected only when `0`).
- **D.4 — `ageThreshold`** → `500000000` (injected only when `0`).
- **D.5 — `indexBloatThreshold`** → `30` (injected only when `0`).
- **D.6 — `scanDuration`** → `2h` (injected only when empty).
- **Contract:** defaults are injected **only** when `recommendationScan.enabled: true` (an `enabled: false`/absent spec gets **no** injection — no-op); an explicit user value for any of the six fields is **never** overwritten (CONTROL — the default fills only a zero/empty field); defaulting never denies admission and increments `cloudberry_webhook_admission_total{webhook="mutating",result="allowed"}` (no dedicated per-field storage metric). **Verified live** against the deployed Vault-PKI mutating webhook (apply minimal `enabled: true` → `GET` → all six defaults persisted).
- **Artifacts**: `internal/webhook/scenario114_defaults_test.go` (unit), `test/cases/scenario114_storage_defaults_cases.go` (catalog), `test/functional/scenario114_storage_defaults_test.go`, `test/integration/scenario114_storage_defaults_test.go`, `test/e2e/scenario114_storage_defaults_e2e_test.go`.

See [spec 13 §Scenario 114](../specifications/13-storage-recommendations-spec.md#scenario-114--mutating-webhook-defaults).

#### Scenario 115 — Enable Storage Management with Full Configuration (C.1/C.3/C.5/R.1/R.5)

The full-block `spec.storage` (`StorageManagementSpec`) reconciliation path for the **storage-recommendations** rules **C.1, C.3, C.5, R.1, R.5**, exercised by `AdminReconciler.reconcileStorage()` (`internal/controller/admin_controller.go`). These `C.*`/`R.*` IDs are **storage-recommendations-scoped**. The scheduled CronJob is built by the NEW `BuildRecommendationScanCronJob` (`internal/builder/recommendation_scan_builder.go`) and ensured/GC'd by `ensureRecommendationScanCronJob` / `removeRecommendationScanCronJob`. The CR applies the full block: `diskMonitoring: true`; `recommendationScan{enabled: true, schedule: "0 3 * * 0", bloatThreshold: 20, skewThreshold: 50, ageThreshold: 500000000, indexBloatThreshold: 30, scanDuration: "2h"}`; `usageReport{enabled: true, monthly: true}`.

- **R.1 — diskMonitoring gate.** `reconcileStorage()` detects `diskMonitoring: true` and **proceeds**; it **early-returns (no-op)** when `spec.storage` is absent or `diskMonitoring` is false (no CronJob, no condition).
- **C.1/C.3 — recommendationScan parse/accept.** The `recommendationScan` config is accepted when enabled (schedule parsed; `RecommendationCount` propagated to status), and the threshold set (`bloatThreshold`/`skewThreshold`/`ageThreshold`/`indexBloatThreshold`/`scanDuration`) is accepted unchanged and passed to the CronJob pod as env vars; per-table `cloudberry_table_bloat_ratio` gauges are published best-effort.
- **C.5 — `<cluster>-recommendation-scan` CronJob.** Created for the `0 3 * * 0` schedule via `BuildRecommendationScanCronJob` + `ensureRecommendationScanCronJob`, and **GC'd** (delete-if-exists) via `removeRecommendationScanCronJob` when the scan is disabled / has no schedule (nil-means-delete). Uses `ForbidConcurrent`, successful/failed history limits `= 3`, `OwnerReferences` to the cluster, and a JobTemplate pod carrying the scan thresholds + duration as env vars (`SCAN_BLOAT_THRESHOLD`/`SCAN_SKEW_THRESHOLD`/`SCAN_AGE_THRESHOLD`/`SCAN_INDEX_BLOAT_THRESHOLD`/`SCAN_DURATION`). **HONEST scaffold:** the pod runs a read-only `SELECT 1` connectivity probe, NOT the real scan SQL — the CronJob proves the schedule is materialized.
- **R.5 — `StorageConfigured=True`.** On a successful storage reconcile the `StorageConfigured` condition is set to `True` with reason `StorageReconciled`; the enabled monthly `usageReport` is accepted; reconcile completes with **no errors**.
- **Metric.** `cloudberry_recommendation_scan_cronjob{cluster,namespace}` gauge — `1` when the CronJob is ensured, `0` when removed (driven by `ensureRecommendationScanCronJob`, called unconditionally every reconcile). **Verified live** against the deployed operator (apply full block → CronJob exists + `StorageConfigured=True` + no errors).
- **Artifacts**: `internal/builder/recommendation_scan_builder.go` (`BuildRecommendationScanCronJob`), `internal/controller/admin_controller.go` (`reconcileStorage`, `ensureRecommendationScanCronJob`/`removeRecommendationScanCronJob`), `internal/builder/scenario115_recommendation_scan_builder_test.go` (builder unit), `internal/controller/scenario115_storage_management_test.go` (controller unit), `test/cases/scenario115_storage_management_cases.go` (catalog), `test/functional/scenario115_storage_management_test.go`, `test/integration/scenario115_storage_management_test.go`, `test/e2e/scenario115_storage_management_e2e_test.go`.

See [spec 13 §Scenario 115](../specifications/13-storage-recommendations-spec.md#scenario-115--enable-storage-management-with-full-configuration).

#### Scenario 116 — Disk Usage Monitoring (Status + Metric) (S.1/M.1/R.2)

Disk usage monitoring for `spec.storage.diskMonitoring`, exercised by the NEW `AdminReconciler.recordDiskUsage()` (`internal/controller/admin_controller.go`) — wired into both `reconcileStorage()` (R.2) and the steady-state `refreshStorageOnSteadyState()` (steady-state growth tracking) — and backed by the NEW DB method `GetDiskUsagePercent()` (`internal/db/client.go`). `recordDiskUsage` mirrors `recordTableBloatRatios` (best-effort, non-fatal, dbFactory client + 10s timeout + span). These `R.*`/`S.*`/`M.*` IDs are **storage-recommendations-scoped**.

- **R.2 — measure each reconcile.** `reconcileStorage()` (and steady-state `refreshStorageOnSteadyState()`) MEASURE disk usage via `recordDiskUsage` → `GetDiskUsagePercent()`, which queries `gp_toolkit.gp_disk_free` and returns the **MAX worst-case** `usage% = 100*(df_total-df_free)/df_total` (clamped `0..100`, divide-by-zero guarded) across segment data volumes.
- **Honest fallback (NO fabrication).** When `gp_toolkit.gp_disk_free` (or an expected column) is absent on this server version (SQLSTATE `42P01`/`42703`), `GetDiskUsagePercent` returns the sentinel `db.ErrDiskUsageUnavailable` and `recordDiskUsage` **SKIPS** — it does not fabricate a value or overwrite the prior status (a transient DB outage never reports a misleading "disk empty" signal).
- **S.1 — `status.diskUsagePercent` populated.** Set to the **current measured value** (no longer a static `0`) and persisted via `patchStatus`; it **tracks growth** as data grows and is never sticky.
- **M.1 — `cloudberry_disk_usage_percent{cluster,namespace}` == status.** The gauge is published FROM THE SAME local measured value as S.1, so the metric **matches** the status. The **`M.1 == S.1` invariant** holds by construction: both derive from the single measured `pct`; the stale `Status.DiskUsagePercent` is never read to publish the gauge. **Verified live** against the deployed operator/cluster (status populated + metric matches), and cross-checked against actual `df` filesystem usage on a segment data volume.
- **Artifacts**: `internal/db/client.go` (`GetDiskUsagePercent`/`diskUsagePercentQuery`, `ErrDiskUsageUnavailable`), `internal/controller/admin_controller.go` (`recordDiskUsage`, wired into `reconcileStorage`/`refreshStorageOnSteadyState`), `internal/db/scenario116_disk_usage_test.go` (DB unit), `internal/controller/scenario116_disk_usage_test.go` (controller unit), `test/cases/scenario116_disk_usage_cases.go` (catalog), `test/functional/scenario116_disk_usage_test.go`, `test/integration/scenario116_disk_usage_test.go`, `test/e2e/scenario116_disk_usage_e2e_test.go`.

See [spec 13 §Scenario 116](../specifications/13-storage-recommendations-spec.md#scenario-116--disk-usage-monitoring-status--metric).

#### Scenario 117 — Recommendation Scan Across All Four Types (S.2/M.2/R.3/R.4/RT.1-4/C.6-9/M.4)

The recommendation scan across the **four** types for `spec.storage.recommendationScan`, exercised by `AdminReconciler.recordRecommendations()` (`internal/controller/admin_controller.go`) — wired into both `reconcileStorage()` (R.3) and the steady-state `refreshStorageOnSteadyState()` — and backed by four NEW THRESHOLD-AWARE DB methods on `internal/db/client.go`. These `S.*`/`M.*`/`R.*`/`RT.*`/`C.*` IDs are **storage-recommendations-scoped**.

- **The four types + thresholds (each gated, injection-safe `$1`):**
  - **RT.1 `bloat` / C.6** — `dead_pct >= bloatThreshold` from `pg_stat_user_tables` (also feeds `cloudberry_table_bloat_ratio`, M.4).
  - **RT.2 `skew` / C.7** — `skccoeff >= skewThreshold` from `gp_toolkit.gp_skew_coefficients`.
  - **RT.3 `age` / C.8** — `age(relfrozenxid) >= ageThreshold` from `pg_class`/`pg_namespace` (REAL freeze age, not a dead-tuple proxy).
  - **RT.4 `index_bloat` / C.9** — estimated `bloat_pct >= indexBloatThreshold` from `pg_class`/`pg_index`/`pg_namespace` (portable estimate, not the old "size > 0" gate).
- **Honest fallback (NO fabrication):** when a `gp_toolkit` view or expected catalog column is unavailable (SQLSTATE `42P01`/`42703`), the affected `Get*` method SKIPS + logs and returns no rows — that type counts `0`; skew never reverts to the row-count proxy, age never to a dead-tuple proxy, index bloat never to the raw size gate.
- **R.3 — four-type scan.** Reconcile reads the four CRD thresholds into one `db.RecommendationThresholds{Bloat,Skew,Age,IndexBloat}` and runs all four scans in a single pass; best-effort and non-fatal (a `Get*` error skips that type without failing the reconcile or fabricating a count).
- **S.2/R.4 + M.2 — `M.2 == count` invariant.** `status.recommendationCount` is set to the CURRENT total (sum of the four per-type active counts, not the stale prior value); `cloudberry_recommendations_total{type}` is set for ALL four types **including `0`** so a cleared/out-of-threshold type resets rather than going stale. The invariant: `status.recommendationCount` equals the sum over types of the value passed to `SetRecommendationsTotal{type}` (both from the SAME per-type counts in one pass).
- **M.4 — `cloudberry_table_bloat_ratio{table}`.** Published from the SAME bloat scan as the `type=bloat` count (one bloat query per reconcile), capped at the top-20 tables.
- **Sub-scenarios:** 117a table bloat (RT.1/C.6/M.4 + CLEARS when bloat drops below threshold); 117b data skew (RT.2/C.7 + APPEARS/DISAPPEARS at the `skewThreshold` boundary); 117c XID age (RT.3/C.8); 117d index bloat (RT.4/C.9). **Verified live** against the deployed operator/cluster (per-type recommendations triggered via DB fixtures; metrics + count populated).
- **Artifacts**: `internal/db/client.go` (the four threshold-aware `Get{Bloat,Skew,Age,IndexBloat}Recommendations` + `RecommendationThresholds`), `internal/controller/admin_controller.go` (`recordRecommendations`, wired into `reconcileStorage`/`refreshStorageOnSteadyState`), `internal/db/scenario117_recommendation_scan_test.go` (DB unit), `internal/controller/scenario117_recommendation_scan_test.go` (controller unit), `test/cases/scenario117_recommendation_scan_cases.go` (catalog), `test/functional/scenario117_recommendation_scan_test.go`, `test/integration/scenario117_recommendation_scan_test.go`, `test/e2e/scenario117_recommendation_scan_e2e_test.go`.

See [spec 13 §Scenario 117](../specifications/13-storage-recommendations-spec.md#scenario-117--recommendation-scan-across-all-four-types).

#### Scenario 118 — Scan Scheduling and Duration Limit (C.5/C.10/M.3 + W.5)

Scan scheduling and the `scanDuration` cap for `spec.storage.recommendationScan`, exercised by `AdminReconciler.recordRecommendations()` / `resolveScanDuration()` (`internal/controller/admin_controller.go`), with the CronJob built by `BuildRecommendationScanCronJob` (Scenario 115) and W.5 in `internal/webhook/validating.go::validateStorageManagement`. These `C.*`/`M.*`/`W.*` IDs are **storage-recommendations-scoped** (W.5 is DISTINCT from the data-loading `W.*` family).

- **118a — schedule fires (C.5/M.3).** The recommendation-scan CronJob `<cluster>-recommendation-scan` is created for the configured `schedule` (via `BuildRecommendationScanCronJob` + `ensureRecommendationScanCronJob`); after each scan in `recordRecommendations` the `cloudberry_recommendation_scan_duration_seconds` histogram is populated via `ObserveRecommendationScanDuration` (M.3).
- **118b — `scanDuration` cap (C.10).** `recordRecommendations` bounds the WHOLE scan with a **single shared** `context.WithTimeout(ctx, resolveScanDuration(scan, logger))` budget across the four `Get*` queries (ONE budget, NOT `4x` the cap). On a deadline-trip mid-run it records **partial results** (only the per-type counts that completed; un-run types count `0`, **no fabrication**), sets `status.recommendationScanTruncated=true`, increments `cloudberry_recommendation_scan_truncated_total{cluster,namespace}`, and sets `status.lastRecommendationScanTime`; the **M.3 duration metric reflects the capped run**.
- **`resolveScanDuration` policy:** empty/unparseable/`<= 0` → `10s` default (preserves historical behavior); `> 24h` → clamp `24h`; otherwise the parsed value **verbatim** (a tiny `10ms` deterministically trips the deadline — no production floor).
- **Never-sticky flag:** `status.recommendationScanTruncated` is set on **every** scan and the field omits `omitempty`, so a prior `true` reliably clears to `false` via the status MergePatch on the next non-truncated scan. `status.lastRecommendationScanTime` is set each scan (capped or complete).
- **W.5 (webhook, defense-in-depth).** An invalid `scanDuration` is rejected: `storage.recommendationScan.scanDuration "<v>" must be a valid Go duration` (in `validateStorageManagement`; `enabled`-gated, webhook-authoritative). It only checks parseability — the clamp/fallback policy lives in `resolveScanDuration`.
- **New status fields + metric:** `status.recommendationScanTruncated` (`bool`), `status.lastRecommendationScanTime` (`*metav1.Time`), `cloudberry_recommendation_scan_truncated_total` (Counter; `cluster`,`namespace`). **Verified live** against the deployed operator/cluster (CronJob + M.3 populated; a tight cap → truncated flag + counter increase).
- **Artifacts**: `internal/controller/admin_controller.go` (`resolveScanDuration`, `recordRecommendations`), `internal/webhook/validating.go` (W.5), `internal/metrics/metrics.go` (`IncRecommendationScanTruncated` / `cloudberry_recommendation_scan_truncated_total`), `api/v1alpha1/types.go` (`RecommendationScanTruncated`/`LastRecommendationScanTime`), `internal/controller/scenario118_scan_duration_test.go` (controller unit), `internal/webhook/scenario118_validation_test.go` (webhook unit), `internal/metrics/scenario118_metrics_test.go` (metrics unit), `test/cases/scenario118_scan_scheduling_cases.go` (catalog), `test/functional/scenario118_scan_scheduling_test.go`, `test/integration/scenario118_scan_scheduling_test.go`, `test/e2e/scenario118_scan_scheduling_e2e_test.go`.

See [spec 13 §Scenario 118](../specifications/13-storage-recommendations-spec.md#scenario-118--scan-scheduling-and-duration-limit).

#### Scenario 119 — All Storage API Endpoints (P.1–P.6)

The six storage REST endpoints for `spec.storage`, implemented in `internal/api/server.go` (the six handlers + their `collect*` helpers) and now returning **REAL data** (no static placeholders) — backed by the DB methods on `internal/db/client.go`. **Verified live** against the deployed operator API. These `P.*` IDs are **storage-recommendations-scoped** (DISTINCT from the data-loading `P.*` family in spec 12).

- **The six endpoints + real-data wiring:**
  - **P.1 `GET …/storage/disk-usage`** → `{cluster, diskUsagePercent, diskUsage:[per-db via GetDiskUsage], diskUsageBySegment:[per-tablespace/segment via GetStorageDiskUsage]}`. `diskUsagePercent` is sourced ONLY from `status.diskUsagePercent`, so **`P.1 == status.diskUsagePercent`** (≡ Scenario 116 `M.1 == S.1`); the breakdowns are purely additive.
  - **P.2 `GET …/storage/tables`** → `{tables:[{schema,table,sizeBytes,sizeHuman,bloatPercent,skewPercent,rowCount}], total}` via the new `GetTables` (`pg_stat_user_tables` + `pg_total_relation_size`; bloat `FLOOR(...)::int`; skew best-effort `gp_toolkit.gp_skew_coefficients` with an HONEST fallback to `0`).
  - **P.3 `GET …/storage/tables/{schema}/{table}`** → `{schema,table,sizeBytes,rowCount,bloatPercent,skewPercent,lastVacuum,lastAnalyze,indexSizes:[{name,sizeBytes,sizeHuman}]}` via `GetTableDetails`. **Bug fix:** bloat AND skew are now both populated (bloat via the `FLOOR(...)::int` NUMERIC-safe cast; skew enriched best-effort from `gp_toolkit`), and `IndexSizes` is added (`collectIndexSizes`).
  - **P.4 `GET …/storage/recommendations`** → `{recommendations:[{type,target:"schema.table",value,ratio,severity,description}], recommendationCount, total}` via the four threshold-aware `Get*`; `recommendationCount` is LIVE when the DB is reachable, else the cached `status.recommendationCount`.
  - **P.5 `POST …/storage/recommendations/scan`** → `202 {status:"scan initiated", cluster}` when `recommendationScan.enabled` (each POST advances the `cloudberry_recommendation_scan_duration_seconds` count, **independent of the cron**); `400 RECOMMENDATION_SCAN_NOT_ENABLED` when disabled.
  - **P.6 `GET …/storage/usage-report`** → `{month, entries, total, usageReportEnabled}` via `GetUsageReport`. **Soft-gated:** disabled → `usageReportEnabled:false` + empty (a READ, NOT a 400); enabled → `entries` + `true`.
- **Auth model:** reads (P.1–P.4, P.6) require **Basic**; the scan POST (P.5) requires **Operator**.
- **Best-effort / non-fatal:** every read collector mirrors `collectDiskUsage` — a nil `dbFactory`, a `NewClient` error, or a query error yields an HONEST **empty** payload with **HTTP 200**, never a `500`; a missing cluster is the only hard error (`404 CLUSTER_NOT_FOUND`). Skew stays an honest `0` (never fabricated, never a row-count proxy) when `gp_toolkit` is absent (SQLSTATE `42P01`/`42703`).
- **Artifacts**: `internal/api/server.go` (the six handlers + `collect*` helpers + `runRecommendationScan`), `internal/db/client.go` (`GetTables`/`collectTableSkew`, `GetTableDetails`/`collectIndexSizes`, `GetUsageReport`), `internal/db/scenario119_storage_api_test.go` (DB unit), `internal/api/scenario119_storage_api_test.go` (API unit), `test/cases/scenario119_storage_api_cases.go` (catalog), `test/functional/scenario119_storage_api_test.go`, `test/integration/scenario119_storage_api_test.go`, `test/e2e/scenario119_storage_api_e2e_test.go`.

See [spec 13 §Scenario 119](../specifications/13-storage-recommendations-spec.md#scenario-119--all-api-endpoints).

#### Scenario 120 — Usage Reporting (C.11/C.13)

Usage reporting for `spec.storage.usageReport`, implemented across `GetUsageReport`/`collectUsageReportTables` (`internal/db/client.go`), `handleGetUsageReport`/`collectUsageReport` (`internal/api/server.go`, endpoint **P.6**), and the `cloudberry-ctl storage usage-report` CLI command with the NEW `--month` flag (`cmd/cloudberry-ctl/main.go::newStorageCmd`). **Verified live** against the deployed operator API + CLI. These `C.*` IDs are **storage-recommendations-scoped**.

- **C.11 — report generation + content.** With `usageReport.enabled: true` (+ `monthly: true`), the operator produces an **ON-DEMAND** monthly usage report (scoped/labeled by `month`) containing **per-table AND per-database** storage consumption. `GetUsageReport(ctx, month)` returns `[]db.UsageReportEntry{Month, Database, SizeBytes, SizeHuman, Connections, Tables:[]TableUsage{Schema,Table,SizeBytes,SizeHuman}}`. The per-table `Tables` is attached to the **CONNECTED-db** entry only (the pgx pool is single-database — honest), best-effort (empty on the honest `42P01`/`42703` fallback or any error), and bounded `LIMIT 50` (largest-first, catalog/internal schemas excluded). HONEST: no persisted month-over-month history, so `growthBytes`/`queryCount` stay `0`.
- **C.13 — dual retrieval.** Retrievable via BOTH the **API** (P.6 `GET …/storage/usage-report` → `200 {cluster, month, entries:[…with tables…], total, usageReportEnabled}`; **Basic** perm; `?month=YYYY-MM` scopes/labels) AND the **CLI** (`cloudberry-ctl storage usage-report --cluster X [--namespace …] [--month YYYY-MM]`, the NEW `--month` flag threading `?month=`).
- **Disabled gate.** `usageReport.enabled: false` (the Scenario 8 reference) → the report is **UNAVAILABLE**: P.6 is **soft-gated** and returns `usageReportEnabled: false` + empty `entries` (a READ, NOT a 400); the CLI surfaces the same payload.
- **Honest fallbacks:** no persisted history → growth/queryCount `0`; single-db pool → `tables[]` only on the connected-db entry; `gp_catalog`/column absent (`42P01`/`42703`) or any tables-query error → empty per-table breakdown (the per-database content still surfaces); DB unreachable at the API → honest empty `entries` (HTTP 200, not 500).
- **Artifacts**: `internal/db/client.go` (`GetUsageReport`, `attachUsageReportTables`/`collectUsageReportTables`/`usageReportTablesQuery`, `UsageReportEntry`/`TableUsage`), `internal/api/server.go` (`handleGetUsageReport`/`collectUsageReport`), `cmd/cloudberry-ctl/main.go` (`newStorageCmd` + the `--month` flag), `internal/db/scenario120_usage_report_test.go` (DB unit), `internal/api/scenario120_usage_report_test.go` (API unit), `cmd/cloudberry-ctl/scenario120_usage_report_test.go` (CLI unit), `test/cases/scenario120_usage_report_cases.go` (catalog), `test/functional/scenario120_usage_report_test.go`, `test/integration/scenario120_usage_report_test.go`, `test/e2e/scenario120_usage_report_e2e_test.go`.

See [spec 13 §Scenario 120](../specifications/13-storage-recommendations-spec.md#scenario-120--usage-reporting).

#### Scenario 121 — All Storage CLI Commands (L.1-L.6)

The six storage CLI commands for `spec.storage`, implemented in `cmd/cloudberry-ctl/main.go::newStorageCmd` as thin clients over the Scenario 119/120 storage API endpoints **P.1–P.6**. **Verified live** against the deployed operator API (all six commands run end-to-end). These storage-recommendations `L.*` IDs are **DISTINCT** from the data-loading `L.1`–`L.16` CLI family (spec 12 / Scenario 108); the two `L.*` namespaces do not overlap.
...
- **The six commands → endpoint mapping:**
  ...
  - **L.3 `storage tables detail --cluster X --schema public --table orders`** → `GET` P.3 `…/storage/tables/{schema}/{table}` (prints table detail).
  ...
  - **L.5 `storage recommendations scan --cluster X`** → `POST` P.5 `…/storage/recommendations/scan` (triggers a best-effort scan; Operator perm at the API).
  - **L.6 `storage usage-report --cluster X --month 2026-05`** → `GET` P.6 `…/storage/usage-report?month=YYYY-MM` (prints the usage report for the selected month).
- **L.3 — flags + positional args.** `storage tables detail` accepts the `--schema`/`--table` flags (canonical form) AND legacy positional `[schema] [table]` args for backward compatibility. `resolveTableDetail` resolves the pair with flags taking precedence when non-empty, else falls back to `args[0]=schema`/`args[1]=table`. A missing schema/table → clear usage error (`schema and table are required (use --schema/--table or positional args)`) **before** any HTTP call (no half-built `/tables//` request).
- **L.6 — `--month` selects the period (honest on-demand labeling).** The optional `--month` flag is encoded (alongside `--namespace`) through a single `url.Values` builder as the `?month=` query param the API honors. The report is computed **ON DEMAND** from live catalog sizes and the month is a label/scope only — HONEST: no persisted month-over-month history, so the report is NOT a historical snapshot (`growthBytes`/`queryCount` stay `0`). Omitting `--month` returns the current/unscoped report.
- **Artifacts**: `cmd/cloudberry-ctl/main.go` (`newStorageCmd` — the six commands L.1–L.6, the L.3 `--schema`/`--table` flags; `resolveTableDetail`), `cmd/cloudberry-ctl/scenario121_storage_cli_test.go` (CLI unit), `test/cases/scenario121_storage_cli_cases.go` (catalog), `test/functional/scenario121_storage_cli_test.go`, `test/integration/scenario121_storage_cli_test.go`, `test/e2e/scenario121_storage_cli_e2e_test.go`.

See [spec 13 §Scenario 121](../specifications/13-storage-recommendations-spec.md#scenario-121--all-cli-commands).

#### Scenario 122 — Disabled States (C.2/C.4/C.12)

The three storage disabled-state behaviors + re-enablement, implemented in `AdminReconciler.reconcileStorage()`/`refreshStorageOnSteadyState()` via the NEW `clearStorageSignals`/`clearRecommendations` helpers (`internal/controller/admin_controller.go`) and the soft-gated `handleGetUsageReport` (`internal/api/server.go`). **Verified live**: each feature disabled → its signal reset; each re-enabled → reactivation. These `C.*` IDs are **storage-recommendations-scoped**.

- **C.2 — `diskMonitoring: false`.** `reconcileStorage()` does **NOT** measure disk usage (the R.1 gate early-returns before `recordDiskUsage`). `status.diskUsagePercent` is **RESET to 0** and the `cloudberry_disk_usage_percent` gauge published **0** via `clearStorageSignals` (an explicit "monitoring off" signal — the operator resets-to-0 on disable, NOT a frozen stale reading; the gauge must not ADVANCE after the flip). The recommendation-scan CronJob is **GC'd**. **Re-enable** → `recordDiskUsage` resumes; status/metric repopulate from the live measurement.
- **C.4 — `recommendationScan.enabled: false`.** **No** scan CronJob/scheduled reconcile; recommendations **NOT produced**. `clearRecommendations` **RESETS** `status.recommendationCount` to **0** and `cloudberry_recommendations_total{type}` to **0** for **all four** types (`bloat`/`skew`/`age`/`index_bloat`), and clears `status.recommendationScanTruncated`. The per-table `cloudberry_table_bloat_ratio` gauge is **NOT cleared** (per-table cardinality the operator doesn't enumerate on disable — an HONEST, documented limitation). `POST …/storage/recommendations/scan` → **`400 RECOMMENDATION_SCAN_NOT_ENABLED`** (explicit-trigger gate); `GET …/storage/recommendations` stays a live read. **Re-enable** → CronJob + recommendations resume.
- **C.12 — `usageReport.enabled: false`.** The usage-report endpoint/CLI **report the feature disabled** — P.6 is a READ, so it is an **API soft-gate**: `200 {usageReportEnabled: false, entries: []}` (NOT a 400); the CLI inherits the same payload. **Re-enable** → `entries` populate + `usageReportEnabled: true`.
- **Reset-to-0 + `omitempty` removal.** On disable the operator deliberately **resets the signal to 0** so `0` is an explicit "feature off" signal (not a frozen value). To make `0` reliably persist, `status.diskUsagePercent` and `status.recommendationCount` had their `omitempty` JSON tag **REMOVED** in `api/v1alpha1/types.go` — a `0` is always serialized in the status MergePatch, so `kubectl get` shows `0` on a disabled cluster (otherwise a `0` would be elided and the stale value would linger). `clearStorageSignals` persists the cleared status ONLY when there was an actual stale value (R6 anti-churn), so the never-configured common case does not churn. Both the spec-driven `reconcileStorage()` R.1 early-return and the steady-state `refreshStorageOnSteadyState()` storage-off block converge on the same helpers.
- **Artifacts**: `internal/controller/admin_controller.go` (`clearStorageSignals`, `clearRecommendations` + wiring), `internal/api/server.go` (`handleGetUsageReport` C.12 soft-gate; `handleTriggerRecommendationScan` C.4 400), `api/v1alpha1/types.go` (`DiskUsagePercent`/`RecommendationCount` with `omitempty` removed), `internal/controller/scenario122_disabled_states_test.go` (controller unit), `internal/api/scenario122_disabled_states_test.go` (API unit), `test/cases/scenario122_disabled_states_cases.go` (catalog), `test/functional/scenario122_disabled_states_test.go`, `test/integration/scenario122_disabled_states_test.go`, `test/e2e/scenario122_disabled_states_e2e_test.go`.

See [spec 13 §Scenario 122](../specifications/13-storage-recommendations-spec.md#scenario-122--disabled-states-c2-c4-c12).
